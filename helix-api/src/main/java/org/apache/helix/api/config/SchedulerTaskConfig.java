...
public class SchedulerTaskConfig {
  ...
  private final Map<PartitionId, IMessage> _innerMessageMap;

  public SchedulerTaskConfig(Map<String, Integer> transitionTimeoutMap,
      Map<PartitionId, IMessage> innerMsgMap) {
   ...
   */
  public <T extends IMessage> T getInnerMessage(PartitionId partitionId) {
    return (T) _innerMessageMap.get(partitionId);
  }
  ...
  public int getTimeout(String transition, PartitionId partitionId) {
    ...
    if (timeout == null) {
      IMessage innerMessage = getInnerMessage(partitionId);
