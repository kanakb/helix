...
import org.apache.helix.api.model.builder.ClusterConstraintsBuilder;
...
public class ClusterConfig {
  ...
  private final Map<ConstraintType, IClusterConstraints> _constraintMap;
  private final Map<StateModelDefId, IStateModelDefinition> _stateModelMap;
  ...
  public ClusterConfig(ClusterId id, Map<ResourceId, ResourceConfig> resourceMap,
      ...
      Map<ConstraintType, IClusterConstraints> constraintMap,
      Map<StateModelDefId, IStateModelDefinition> stateModelMap, UserConfig userConfig,
  ...
  public int getTransitionConstraint(Scope<?> scope, StateModelDefId stateModelDefId,
    ...
    IClusterConstraints transitionConstraints =
    ...
    Set<IConstraintItem> matches = transitionConstraints.match(matchAttributes);
    int value = Integer.MAX_VALUE;
    for (IConstraintItem item : matches) {
   ...
   */
  public Map<StateModelDefId, IStateModelDefinition> getStateModelMap() {
  ...
  public static class Delta {
    ...
    private ClusterConfigBuilder _builder = ClusterConfigBuilder.newInstance();
    ...
    public Delta addTransitionConstraint(Scope<?> scope, StateModelDefId stateModelDefId,
        ITransition transition, int maxInFlightTransitions) {
    ...
    public ClusterConfig mergeInto(ClusterConfig orig) {
      ...
      ClusterConfigBuilder builder =
          ClusterConfigBuilder.newInstance().withClusterId(orig.getId()).addResources(orig.getResourceMap().values())
      ...
      for (ConstraintType type : ConstraintType.values()) {
        IClusterConstraints constraints;
        if (orig.getConstraintMap().containsKey(type)) {
          constraints = orig.getConstraintMap().get(type);
        } else {
          constraints = ClusterConstraintsBuilder.newInstance().withType(type).build();
        }
        // add new constraints
        if (deltaConfig.getConstraintMap().containsKey(type)) {
          IClusterConstraints deltaConstraints = deltaConfig.getConstraintMap().get(type);
          for (ConstraintId constraintId : deltaConstraints.getConstraintItems().keySet()) {
            IConstraintItem constraintItem = deltaConstraints.getConstraintItem(constraintId);
